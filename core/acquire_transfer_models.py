...
from util.ordinary import get_transfer_model_name, get_bottleneck_name, dump_as_pickle, get_delete_rate_name
from util.transfer_util import save_filtered_bottleneck_data, get_svm_classifier, get_dense_classifier, \
    get_pool_classifier, save_bottleneck_data


def acquire(target_ds=None, parent_model=None):
    ...
    bottleneck_features_train = save_bottleneck_data(getSourceModel(parent_model),
                                                     train_generator, nb_train_samples,
                                                     batch_size, split='train',
                                                     target_ds=target_ds)
    bottleneck_features_valid = save_bottleneck_data(getSourceModel(parent_model),
