...
for m in all_model_names:
    ...
    for lb in [0, 1]:
        ...
        for x in sx:
            ...
            nx += 1
            _, _, all_features = find_local_feature(hidden_val, feature_neurons, similarity_threshold=1.0)

            for f in all_features:
                _f = (f[0], f[1])
                if _f not in feature_count:
                    feature_count[_f] = 0.0
                feature_count[_f] += f[2]
        ...
        if PRINT:
            print('Ranked features for class ' + str(lb))
