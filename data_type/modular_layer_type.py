    ...
    def __init__(self, layer, timestep=None):
        ...
        if self.type != LayerType.Activation:
            ...
            if self.type == LayerType.Dense or self.type == LayerType.TimeDistributed:
                ...
                self.active_count = np.array([0.0] * self.num_node).reshape(1, self.num_node)
                self.median_node_val = np.array([0.0] * self.num_node).reshape(1, self.num_node)
                self.most_active_neuron_mask = np.array([False] * self.num_node).reshape(1, self.num_node)
                self.weight_importance_count = np.array([0] * self.W.shape[0]).reshape(1, self.W.shape[0])
        ...
        if not Constants.UNROLL_RNN and not self.return_sequence:
            if h_t_previous is None:
                ...
                if self.type == LayerType.LSTM:
                    self.cell_state = np.array([0] * self.num_node).reshape(1, self.num_node)
